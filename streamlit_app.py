...
def main():
    ...
    with tab_review:
        ...
        if not notes_data:
            ...
            search = st.text_input("\U0001f50d Search notes:", placeholder="Search by routine, dancer, or staff name...")
            ...
            for num, title, dancers in SHOW_ORDER:
                ...
                if key in notes_data and notes_data[key]:
                    ...
                    with st.expander(f"\U0001f3b5 {display_label} ({len(notes_data[key])} note{'s' if len(notes_data[key]) != 1 else ''})"):
